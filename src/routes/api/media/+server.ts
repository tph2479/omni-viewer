...
export async function GET({ url, request }: RequestEvent) {
	try {
		...
		if (isZipPath) {
			...
			return new Response(null, { status: 307, headers: { 'Location': `/api/ebook?${url.searchParams.toString()}` } });
		}
		...
		if (getMetadataOnly) {
			return handleMetadata(absolutePath, normalizedPath, stat, ext, isHeic, request.signal, isRetry);
		}

		if (isThumbnail) {
			const thumbPath = await getThumbnailPath(absolutePath, stat.mtimeMs);
			if (!fs.existsSync(thumbPath)) {
				const success = await generateThumbnail(absolutePath, thumbPath, stat.mtimeMs, request.signal);
		...
		if (isVideo || isAudio) {
			return handleStream(absolutePath, stat, contentType, request.headers.get('range'), request.signal);
		}

		return handleImage(absolutePath, stat, contentType, isHeic, request.signal, isRetry);
	} catch (err: any) {
