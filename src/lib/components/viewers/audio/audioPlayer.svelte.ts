export function createAudioController() {
  ...
  function updateExcitement() {
    ...
    let bassSum = 0;
    for (let i = 0; i < 8; i++) bassSum += s.dataArray[i];
    let midSum = 0;
    for (let i = 10; i < 50; i++) midSum += s.dataArray[i];
    let trebleSum = 0;
    for (let i = 60; i < 110; i++) trebleSum += s.dataArray[i];
    ...
    const targetVal = Math.min(
      1.0,
      (currentMid * 0.7 + currentTreble * 0.3) * 1.4,
    );
    ...
    const baseline =
      s.energyHistory.reduce((a, b) => a + b, 0) / s.energyHistory.length;
    ...
    s.isClimax =
      (energyFactor > 1.0 ||
        fluxFactor > 1.4 ||
        (currentBass > 0.7 && currentTreble > 0.7)) &&
      rawVal > 0.15;
  ...
  function drawGraph() {
    ...
    for (let i = 0; i < currentCount - 1; i++) {
      ...
      if (i === 0) {
        ...
        // Vẽ đường cong qua điểm giữa thay vì nối thẳng
