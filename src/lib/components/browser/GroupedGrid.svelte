<script lang="ts">
    ...
    const groups = ["folders", "images", "cbz", "pdf", "epub", "audio", "videos"];
</script>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] sm:grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3 sm:gap-4">
                        ...
                        {#if groupInfo && groupInfo.total > 11}
                            <button
                                class="relative aspect-square rounded-2xl overflow-hidden transition-all duration-500 cursor-pointer group w-full"
                                onclick={() => actions.openGroup?.(groupKey)}
                            >
                                <!-- Background Layer -->
                                <div class="absolute inset-0 bg-surface-100/50 dark:bg-surface-800/50 backdrop-blur-sm border-2 border-dashed border-primary-500/20 group-hover:border-primary-500/50 group-hover:bg-primary-500/5 transition-all duration-500 rounded-2xl"></div>
                                
                                <!-- Content Layer -->
                                <div class="relative z-10 flex flex-col items-center justify-center p-4">
                                    <div
                                        class="bg-primary-500/10 p-5 rounded-full group-hover:bg-primary-500/20 group-hover:scale-110 transition-all duration-500 shadow-sm group-hover:shadow-lg border border-primary-500/20"
                                    >
                                        <ArrowRight
                                            size={42}
                                    ...
                                    <div class="mt-4 flex flex-col items-center gap-1">
                                        <span class="text-[11px] font-black text-primary-500 uppercase tracking-[0.2em] group-hover:tracking-[0.3em] transition-all duration-500">
                                            View All
                                        </span>
                                        <span class="badge variant-soft-primary px-2 py-0.5 rounded-full text-[9px] font-bold opacity-60 group-hover:opacity-100 transition-opacity">
                                            +{groupInfo.total - groupInfo.items.length} items
                                        </span>
                                    </div>
                                </div>
