...
export type MediaType = 'image' | 'video' | 'audio' | 'pdf' | 'epub' | 'archive' | 'directory' | 'unknown';
...
export type DirectoryEntry = {
    name: string;
    path: string;
    mediaType: 'directory' | 'archive' | 'media' | 'unknown';
    isExecutable?: boolean;
};

export type CoverFolder = { name: string; path: string; coverPath: string };
export type PendingFile = { path: string; type: "media" | "archive" | "pdf" } | null;
