...
import { isArchiveFile } from '$lib/shared/utils/fileUtils';
...
export function createWebtoonController(folderPath: string, contextPath: string = "", actions: any) {
	...
	async function loadSiblings() {
		...
		if (s.contextPath && isArchiveFile(s.contextPath)) {
		...
		try {
			...
			// Use noGroup=true to get a flat list even if folders and archives are mixed
			...
			if (data.items) {
				// Filter for books/chapters (both archive files and folders with images)
				s.siblings = (data.items as MediaFile[]).filter(item => {
					const itemHasImages = !!item.containsImages;
					const isArchive = item.mediaType === 'archive' || isArchiveFile(item.name);
					return itemHasImages || isArchive;
				});
				refreshCurrentIndex();
			}
		} catch (e) {
	...
	async function goToIndex(index: number) {
		if (index >= 0 && index < s.siblings.length) {
			...
			if (isArchiveFile(path)) {
				actions.openArchiveInWebtoon(path, s.contextPath);
			}
			s.isTocOpen = false;
			await tick();
