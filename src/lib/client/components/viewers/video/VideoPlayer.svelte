<script lang="ts">
	...
	import { formatBytes, formatDateTime } from '$lib/shared/utils/formatters';
	import { isVideoFile } from '$lib/shared/utils/fileUtils';
	...
	$effect(() => {
		if (isModalOpen && currentItem && currentItem.path !== s.lastMetadataPath) {
			...
			api.getMediaMetadata(currentItem.path, { cacheVersion: cacheVersion.value, signal: controller.signal })
				.then(data => {
