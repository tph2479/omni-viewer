<script lang="ts">
    ...
    import { useMetadataFetch } from "$lib/client/actions/useMetadataFetch";
    import FolderCard from "./variants/FolderCard.svelte";
    import ZipCard from "./variants/ZipCard.svelte";
    import ArchiveCard from "./variants/ArchiveCard.svelte";
    ...
    type FileActions = {
        ...
        openArchive: (path: string, context?: string) => void;
    ...
    let thumbnailUrl = $derived.by(() => {
        ...
        if (isArchiveFile(item.name) || item.mediaType === "archive") {
    ...
    function handleCardClick() {
        ...
        if (isArchiveFile(item.name) || item.mediaType === "archive") {
            ...
            actions.openArchive(item.path, parentPath);
            return;
        }
        actions.openModal(index);
    }

    function handleOpenArchive(targetPath: string, context: string) {
        actions.openArchive(targetPath, context);
    }
</script>
...
<div class="group flex flex-col">
    ...
    >
        {#if item.mediaType === "directory"}
            <FolderCard {item} {thumbnailUrl} onOpenArchive={handleOpenArchive} />
        {:else if isZipFile(item.name)}
            <ZipCard {highlightedBg} />
        {:else if isArchiveFile(item.name) || item.mediaType === "archive"}
            <ArchiveCard {item} {thumbnailUrl} {fetchPriority} {highlightedBg} />
