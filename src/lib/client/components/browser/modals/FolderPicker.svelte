<script lang="ts">
    ...
    import type { DirectoryEntry } from "$lib/client/stores/browser/types";
    ...
    let {
        ...
        onOpenFile?: (path: string, type: "media" | "archive") => void;
        isFileMode?: boolean;
    } = $props();
        ...
        try {
            ...
            subdirectoryEntries = (data.directories as any[]).map(
                (d) => {
                    ...
                    const isArchive = d.isArchive;
                    const isMedia = d.isMedia;
                    const isExecutable = d.isExecutable;
                    let mediaType: 'directory' | 'archive' | 'media' | 'unknown' = 'unknown';
    ...
    function handleEntryClick(dir: DirectoryEntry) {
        ...
        // Default behavior for non-file mode (media/archive handling)
        const mt = dir.mediaType || 'unknown';
        if (mt === 'archive') {
            folderPath = dir.path;
            isFolderPickerOpen = false;
            onOpenFile?.(dir.path, "archive");
            onSelect(dir.path);
            return;
        }
        if (mt === 'media') {
